...
###     Alternatively, you can also load the list of ase.Atoms object
###          with ase.io.read directly
...
config = {
    ...
    "optim": {
        "device": "cpu",
        "force_coefficient": 0.05,  # for S2EF
        "lr": 1e-2,
        "batch_size": 16,
        "epochs": 100,
    },
...
true_forces = np.array([image.get_forces() for image in images]).reshape(
    -1,
)
pred_forces = np.array(predictions["forces"]).reshape(
    -1,
)

print("Force MSE:", np.mean((true_forces - pred_forces) ** 2))
