...
void find_quaternion(double *source_vec, double *target_vec, double *quaternion){
    double source_vec_mag = magnitude(source_vec);
    double target_vec_mag = magnitude(target_vec);

    double source_vec_norm[3] = {source_vec[0] / source_vec_mag, source_vec[1] / source_vec_mag, source_vec[2] / source_vec_mag};
    double target_vec_norm[3] = {target_vec[0] / target_vec_mag, target_vec[1] / target_vec_mag, target_vec[2] / target_vec_mag};

    //if parallel, no rotation
    if (dot_product(source_vec_norm, target_vec_norm) >= 0.9999){
        quaternion[0] = 1.0;
        quaternion[1] = 0.0;
        quaternion[2] = 0.0;
        quaternion[3] = 0.0;
    }
    //if anti-parallel, 180 rotation along one axis
    else if (dot_product(source_vec_norm, target_vec_norm) <= -0.9999){
        double x_axis[3] = {1.0, 0.0, 0.0};//, y_axis[3] = {0.0, 1.0, 0.0}, z_axis[3] = {0.0, 0.0, 1.0};
        //if NOT parallel/anti-parallel with x, rotate 180 along x axis
        if (dot_product(source_vec_norm, x_axis) < 0.9999 || dot_product(source_vec_norm, x_axis) > -0.9999){
            quaternion[0] = 0.0;
            quaternion[1] = 1.0;
            quaternion[2] = 0.0;
            quaternion[3] = 0.0;
    ...
    // if not parallel, find quaternion
    else {

        quaternion[0] = 1 + dot_product(source_vec_norm, target_vec_norm);
        double cross_product_res[3];
        cross_product(source_vec_norm, target_vec_norm, cross_product_res);
        quaternion[1] = cross_product_res[0];
        quaternion[2] = cross_product_res[1];
        quaternion[3] = cross_product_res[2];
...
// void find_quaternion_consecutive_six(double *vec_six, double *quaternion1_result, double *quaternion2_result){
...
void apply_quaternion(double *vec, double *rot_quaternion){
    ...
    hamilton_product(result1, rot_quaternion_inv, result2);
    ...
    //delete [] result1;
    //delete [] result2;
    //delete [] rot_quaternion_inv;
    //delete [] vec_quaternion;
}

void apply_quaternion_six(double *vec_six, double *rot_quaternion){
    //assume vec_six = {xy, xz, yx, yz, zx, zy}
    double major_axis[3], minor_axis[3];

    major_axis[0] = vec_six[0];
    major_axis[1] = vec_six[1];
    major_axis[2] = vec_six[2];

    minor_axis[0] = vec_six[3];
    minor_axis[1] = vec_six[4];
    minor_axis[2] = vec_six[5];

    apply_quaternion(major_axis, rot_quaternion);
    apply_quaternion(minor_axis, rot_quaternion);

    vec_six[0] = major_axis[0];
    vec_six[1] = major_axis[1];
    vec_six[2] = major_axis[2];
    vec_six[3] = minor_axis[0];
    vec_six[4] = minor_axis[1];
    vec_six[5] = minor_axis[2];
    
}

void apply_quaternion_six_consecutive(double *vec_six, double *rot_quaternion1, double *rot_quaternion2){
    //assume vec_six = {xy, xz, yx, yz, zx, zy}
    double major_axis[3], minor_axis[3];
    ...
    apply_quaternion(major_axis, rot_quaternion1);
    apply_quaternion(minor_axis, rot_quaternion1);

    apply_quaternion(major_axis, rot_quaternion2);
    apply_quaternion(minor_axis, rot_quaternion2);
...
void convert_six_to_axis(double *vec_six){
    //assume vec_six = {xy, yx, xz, zx, yz, yz}
    double major_axis[3], minor_axis[3];

    major_axis[0] = sqrt(vec_six[4] * vec_six[4] + vec_six[5] * vec_six[5]);
    major_axis[1] = sqrt(vec_six[2] * vec_six[2] + vec_six[3] * vec_six[3]);
    major_axis[2] = sqrt(vec_six[0] * vec_six[0] + vec_six[1] * vec_six[1]);

    minor_axis[0] = sqrt(vec_six[1] * vec_six[1] + vec_six[3] * vec_six[3]);
    minor_axis[1] = sqrt(vec_six[0] * vec_six[0] + vec_six[5] * vec_six[5]);
    minor_axis[2] = sqrt(vec_six[2] * vec_six[2] + vec_six[4] * vec_six[4]);
