...
class BaseDescriptor(ABC):
    def __init__(self):
        ...
        # elements to be fingerprinted (can be ghost atoms)
        self.fp_elements = []

        # elements that the fingerprints are based on
        self.ref_elements = []
    ...
    def _compute_fingerprints_back(
    ...
    def _compute_fingerprints_nodb_back(
        self, image, image_db_filename, calc_derivatives, save_fps, cores, log
    ):
        descriptor_list = []
    ...
    def _compute_fingerprints(
        ...
        with h5py.File(image_db_filename, "a") as db:
            ...
            fp_atom_indices_arr = np.where(np.isin(atom_indices, self.fp_element_indices))[0]
            image_fp_array = temp_image_fp_array[fp_atom_indices_arr]

            image_dict["atomic_numbers"] = list_symbols_to_indices(symbol_arr[fp_atom_indices_arr])
            image_dict["num_atoms"] = num_fp_atoms
