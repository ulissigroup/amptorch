...
def test_delta_ml():
    ...
    target_ref = energies[0]
    lj_ref = lj_energies[0]
    delta_target = energies - target_ref
    delta_lj = lj_energies - lj_ref
    actual_delta_energies = delta_target - delta_lj
    actual_delta_forces = forces - np.concatenate(np.array(lj_forces))
    ...
    model_energies = training_data.energy_dataset.numpy()
    for i, k in zip(actual_delta_energies, model_energies):
        assert(float(abs(round(i, 5) - round(k, 5))) <= 1e-4), 'Delta energies do not match!'
    new_model_forces = np.array([])
    model_forces = training_data.forces_dataset
    for i in model_forces:
        new_model_forces = np.append(new_model_forces, i.numpy())
    for j, l in zip(new_model_forces, actual_delta_forces[0]):
        assert(float(abs(round(j, 5) - round(l, 5))) <= 1e-4), 'Delta forces do not match!'
    ...
    calc.train(overwrite=True)
    num_of_atoms = 29
    pred_energies = np.array([calc.get_potential_energy(image) for image in images])
    pred_forces = np.concatenate(np.array([calc.get_forces(image) for image in
        images]))
    energy_rmse = np.sqrt(
            (((pred_energies - energies) / num_of_atoms) ** 2).sum() / len(images)
        )
    force_rmse = np.sqrt(
        (((pred_forces - forces)) ** 2).sum() / (3 * num_of_atoms * len(images))
    )
    last_energy_score = net.history[-1]["energy_score"]
    assert round(energy_rmse, 4) <= round(
            last_energy_score, 4
