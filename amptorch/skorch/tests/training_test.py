...
import numpy as np
from amptorch.skorch.model_skorch import CustomLoss, FullNN
from amptorch.skorch import AMP
from amptorch.analysis import parity_plot
from amptorch.gaussian import Gaussian
from amptorch.skorch.skorch_data import TestDataset, collate_amp, AtomsDataset
...
def test_training():
    ...
    forcetraining=True
    training_data = AtomsDataset(images, Gaussian, Gs,
            forcetraining=forcetraining, label=label, cores=1, lj_data=None)
    ...
    net = NeuralNetRegressor(
        module=FullNN(unique_atoms, [fp_length, 2, 2], device, forcetraining=forcetraining),
    ...
    train_hashes = list(training_data.hashed_images.keys())
    train_fp_hashes = {}
    ...
    dataset = TestDataset(
        ...
        Gs=calc.training_data.Gs,
        fprange=calc.fprange,
    )
