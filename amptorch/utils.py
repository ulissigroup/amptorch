...
class Polynomial(object):
    ...
    def __call__(self, Rij):
        ...
        if Rij > self.Rc:
            ...
            value = 1. + self.gamma * np.power(Rij / self.Rc, self.gamma+1) - \
                (self.gamma + 1) * np.power(Rij / self.Rc, self.gamma)
            return value
    ...
    def prime(self, Rij):
        ...
        if Rij > self.Rc:
            ...
            value = (self.gamma * (self.gamma + 1) / self.Rc) * \
                (np.power(Rij / self.Rc, self.gamma) -
                 np.power(Rij / self.Rc, self.gamma - 1))
            return value
