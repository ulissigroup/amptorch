...
def compute_test_stats(calc, test_images, num_of_atoms):
    energies = np.array([image.get_potential_energy() for image in test_images])
    forces = np.concatenate(np.array([image.get_forces() for image in test_images]))
    pred_energies = np.array(
        [calc.get_potential_energy(image) for image in test_images]
    )
    pred_forces = np.concatenate(
        np.array([calc.get_forces(image) for image in test_images])
    )

    energy_rmse = np.sqrt(
        (((pred_energies - energies) / num_of_atoms) ** 2).sum() / len(test_images)
    )

    force_rmse = np.sqrt(
        (((pred_forces - forces)) ** 2).sum() / (3 * num_of_atoms * len(test_images))
    )
...
def mlmd_run(
    ...
    class train_end_load_best_valid_loss(skorch.callbacks.base.Callback):
        def on_train_end(self, net, X, y):
            net.load_params("./results/checkpoints/{}_params.pt".format(basename))
...
def iterative_sampler(
    ...
    # Initial ML/ML-LJ sample images
    if lj:
        name = filename + "_LJ"
    else:
        name = filename
...
test_set = ase.io.read("../datasets/COCu_ber_100ps_300K.traj", "2000:3000:5")
...
basename = "COCu_rngmd_nolj_200s_verify_oldcommitt"
iterative_sampler(
    images=ber_images,
    count=2000,
    sample=num_samples,
    filename=basename,
    file_dir="./ber_results/",
    iteration=3,
    lj=False,
    temp=300,
    Gs=Gs,
